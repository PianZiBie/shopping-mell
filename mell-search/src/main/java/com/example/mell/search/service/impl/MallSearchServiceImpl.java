...
import com.example.mell.search.vo.AttrResponseVo;
import com.example.mell.search.vo.SearchParam;
import com.example.mell.search.vo.SearchResult;
import com.example.mell.search.feign.ProductFeignService;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            //2、执行检索请求
            SearchResponse response = esRestClient.search(searchRequest, mellElasticSearchConfig.COMM_OPTIONS);
    ...
    private SearchResult buildSearchResult(SearchResponse response,SearchParam param) {
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                navVo.setLink("http://search.mell.com/list.html?" + replace);
